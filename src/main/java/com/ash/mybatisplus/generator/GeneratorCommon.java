package com.ash.mybatisplus.generator;
...
//        dsc.setDriverName("com.alibaba.druid.pool.DruidDataSource"); //alibaba 连接池
        dsc.setDriverName("com.mysql.cj.jdbc.Driver"); //mysql8.0使用
        dsc.setUsername("root");
        dsc.setPassword("root");
        dsc.setUrl("jdbc:mysql://192.168.1.21:3306/ksg-xsxy2?useUnicode=true&characterEncoding=utf-8&serverTimezone=Asia/Shanghai");
        mpg.setDataSource(dsc);

        // 策略配置
        StrategyConfig strategy = new StrategyConfig();
        //strategy.setCapitalMode(true);// 全局大写命名 ORACLE 注意
        strategy.setTablePrefix("x_");// 此处可以修改为您的表前缀
        strategy.setNaming(NamingStrategy.underline_to_camel);// 表名生成策略
        strategy.setColumnNaming(NamingStrategy.underline_to_camel);//采用驼峰映射
        strategy.setEntityLombokModel(true);//【实体】是否为lombok模型（默认 false）
        strategy.setRestControllerStyle(true);
        strategy.setInclude(scanner("表名，多个英文逗号分割").split(",")); // 需要生成的表.如果需要生成所有的, 注释掉此行就可以
        ...
        // 包配置com.bituo.xsxy.jpa.bean
        PackageConfig pc = new PackageConfig();
        pc.setParent("com.bituo.xsxy.resume");// 自定义包路径
        pc.setController("controller");// 这里是控制器包名，默认 web
        pc.setMapper("mapper");// 设置Mapper包名，默认mapper
        pc.setService("service");// 设置Service包名，默认service
        pc.setEntity("entity.bean");// 设置Entity包名，默认entity,继承的父类  已序列化
